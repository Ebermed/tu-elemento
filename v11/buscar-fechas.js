/** TU ELEMENTO — Buscar una fecha · Artx 1.1
 *
...
(function(){
  ...
  function natalActual(){
    var id=perfilId();
    if(!id||typeof leerPerfil!=='function'||typeof cartaDesdePerfil!=='function')return null;
    try{
      var p=leerPerfil(id),c=p?cartaDesdePerfil(p):null;
  ...
  function htmlPanel(){
    ...
    return '<section class="buscarPanel">'+
      ...
      '<p class="sub">Elige lo que quieres hacer y mira qué días del año encajan mejor con esa actividad.</p>'+
  ...
  function pintarPerfilEstado(){
    ...
    var n=natalActual();
    if(n&&n.perfil&&n.carta){
      var nom=n.perfil.tipo==='yo'?'tu carta':(n.perfil.nombre||'esta carta');
      e.innerHTML='<span>La búsqueda está personalizada para <strong>'+esc(nom)+'</strong> y también cruza cada fecha con Tu origen: '+esc(n.carta.pilares.anio.rama.animal)+'.</span>';
    }else{
      e.innerHTML='<span>Lectura general. Si eliges una carta en el selector de arriba, esta búsqueda también cruza cada fecha con Tu origen.</span>';
    }
  }
  ...
  async function renderAnio(){
    ...
    construirShell();pintarPerfilEstado();actualizarAyuda();
  ...
  var perfil=q('#perfilCal');if(perfil)perfil.addEventListener('change',function(){if(activo)renderAnio();});
