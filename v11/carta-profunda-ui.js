(function (raiz) {
  ...
  function cta(id,v,texto){return '<a class="btn fantasma" href="'+href(id,v)+'">'+esc(texto)+'</a>';}
  ...
  function resumenGeneral(perfil,carta,lec,a){
    ...
    var pilarIntro=lec.sinHora?'La carta tiene tres pilares calculados. Con la hora de nacimiento también aparece Tu futuro y Tu brújula.':'Tus cuatro pilares miran zonas distintas de tu vida. A ellos se suman Tu brújula y Tu punto de partida.';
    ...
    var nudoIntro=nudo.presente?('El patrón de fondo se concentra primero en '+nudo.items[0].titulo.toLowerCase()+'. La idea es observar qué se repite ahí y darle otra respuesta práctica.'):'La carta no coloca un vacío dentro de tus pilares natales, así que el nudo de fondo no domina esta lectura.';
    ...
    var recursoIntro='El método de balance prioriza '+cap(recurso.elemento)+'. Aquí lo traducimos a una conducta concreta que puedes practicar, no a una promesa de que algo externo vaya a cambiar.';

    var html='<div class="cpVista">'+cab('Tu carta, en corto','Esta es la vista general. Resume las capas que más destacan y te deja entrar a cada una cuando quieras ver de dónde sale la lectura.')+
      ...
      '<a class="cpResumen cpResumen--mezcla" href="'+href(id,'mezcla')+'"><span class="tag">Conteo elemental</span><h3>Tu mezcla</h3><p>'+esc(cap(dom)+' ocupa más espacio: '+mez.dominanteValor+' de '+mez.total+' caracteres. '+mez.lecturaDominante.fortaleza)+'</p><b>Ver tu mezcla</b></a>'+
  ...
  function mezclaHTML(perfil,a){
    ...
    return '<div class="cpVista">'+volver(id)+cab('Tu mezcla','Aquí contamos los cinco elementos dentro de los ocho caracteres de la carta. El conteo describe qué funciones tienen más espacio y cuáles suelen necesitar más intención; no busca que todo quede perfectamente igual.')+
      '<section class="cpHoja"><h3>Así se reparte tu carta</h3><p class="cpSuave">Cada pilar aporta un tallo y una rama. Por eso una carta completa suma ocho piezas.</p><div class="cpElementos">'+barras+'</div><div class="cpDosColumnas">'+
  ...
  function perfilesHTML(perfil,a){
    ...
    return '<div class="cpVista">'+volver(id)+cab('Tus perfiles','Los perfiles salen de comparar cada tallo de la carta con tu Día Maestro. Hablan de maneras distintas de vincularte, expresarte, manejar recursos, responder a presión y aprender.')+
      ...
      '<section class="cpHoja"><h3>Los perfiles que aparecen en tu carta</h3><p class="cpSuave">Este conteo suma tallos visibles y tallos ocultos. Sirve para ver repetición estructural; por eso mostramos apariciones en vez de convertirlas en un porcentaje psicológico.</p><div class="cpPerfilLista">'+cards+'</div></section></div>';
  }
  ...
  function pilarExtraHTML(nombre,extra,tipo){
    ...
    var intro=tipo==='brujula'?'Este pilar funciona como una referencia de regreso: el método lo usa para observar qué pasa cuando necesitas volver a tu propio criterio.':'Este pilar mira el material de arranque: rasgos y tendencias que el método coloca antes de la adaptación que viene con la experiencia.';
  ...
  function pilaresHTML(perfil,lec,a){
    ...
    return '<div class="cpVista">'+volver(id)+cab('Tus pilares','Cada pilar mira una zona distinta. Los cuatro pilares natales salen directamente de año, mes, día y hora; Tu brújula y Tu punto de partida son dos cálculos complementarios construidos a partir de esos mismos datos.')+
      ...
      (lec.tensiones&&lec.tensiones.length?'<section class="cpHoja"><h3>Cuando dos zonas piden cosas distintas</h3><p class="cpSuave">Los roces y enlaces entre pilares sirven para entender por qué una misma decisión puede sentirse fácil en un contexto y más compleja en otro.</p>'+lec.tensiones.map(function(t){return '<div class="cpAccion"><b>'+esc(t.detalle)+'.</b>&nbsp; '+esc(t.texto)+'</div>';}).join('')+'</section>':'')+'</div>';
  }
  ...
  function vaciosHTML(perfil,a){
    ...
    return '<div class="cpVista">'+volver(id)+cab('Tus vacíos','El ciclo de 60 combina diez tallos con doce ramas. En cada bloque quedan dos ramas sin tallo; esas son tus ramas vacías. La lectura se vuelve personal cuando una de ellas ocupa uno de tus pilares.')+
      '<section class="cpHoja"><h3>Tus dos ramas vacías</h3>'+ramas+'<p class="cpSuave">Un vacío no significa que una parte de tu vida esté condenada ni que “falte” algo en ti. Aquí lo usamos como una señal de aprendizaje construido.</p>'+presentes+remate('Lo que no viene dado también se puede construir, y a veces termina siendo de lo que más conoces por experiencia propia.')+'</section></div>';
  }
  ...
  function nudoHTML(perfil,a){
    ...
    if(!n.presente){return '<div class="cpVista">'+volver(id)+cab('Tu nudo de fondo','Esta capa se construye cuando una de tus ramas vacías cae dentro de un pilar natal. En tu carta eso no sucede, así que aquí no vamos a inventarte un problema que el cálculo no está marcando.')+'<section class="cpHoja"><h3>Esta parte queda tranquila</h3><p>Tus vacíos existen en el ciclo, pero no ocupan Tu origen, Tu trayectoria ni Tu futuro. Puedes seguir usando la lectura de vacíos como contexto técnico sin convertirla en el centro de tu carta.</p>'+remate('Una lectura también es útil cuando sabe qué no necesita dramatizar.')+'</section></div>';}
    var items=n.items.map(function(x){var d=x.dinamica,cp=x.perfilCopy||{},ec=raiz.TE_ELEMENTO_COPY&&raiz.TE_ELEMENTO_COPY[x.tallo.elemento],pr=NUDO_PRACTICA[x.pilar]||[];return '<article class="cpNudo"><h3>'+esc(x.titulo)+'</h3><p class="meta">'+esc(nombreAnimal(x.rama)+' · '+nombreTallo(x.tallo)+(d?' · '+d.nombre:''))+'</p><h4>Qué patrón estamos mirando</h4><p>El vacío cae en '+esc(x.tema)+'. Eso vuelve especialmente útil observar qué haces cuando esa zona se siente poco definida o cuando esperas que alguien más te dé una respuesta que todavía estás construyendo.</p>'+(cp.cuidado?'<h4>Cómo se puede trabar</h4><p>'+esc(cp.cuidado)+'</p>':'')+(x.tensiones&&x.tensiones.length?'<p class="cpSuave">Además, este pilar tiene '+x.tensiones.map(function(t){return t.detalle.toLowerCase();}).join(' y ')+'. Eso puede hacer que el patrón aparezca con más claridad cuando dos zonas de tu vida piden ritmos distintos.</p>':'')+'<h4>Qué puedes probar</h4>'+(pr[0]?accion(pr[0]):'')+(pr[1]?accion(pr[1]):'')+(ec?accion(ec.probar[0]):'')+'</article>';}).join('');
    return '<div class="cpVista">'+volver(id)+cab('Tu nudo de fondo','Esta lectura toma tus vacíos y los cruza con el pilar donde aparecen, el elemento que llevan, el perfil que los acompaña y las relaciones internas de la carta. La intención es localizar un patrón repetible, no ponerle una etiqueta dramática a tu pasado.')+
      '<section class="cpHoja"><p class="cpSuave">El manual original llama a esta capa “Trauma Raíz”. Tu Elemento usa “nudo de fondo” porque el cálculo señala un tema de aprendizaje; no diagnostica un trauma psicológico.</p>'+items+remate('El objetivo no es encontrar una explicación total de tu pasado. Es reconocer qué respuesta sigues repitiendo y decidir si todavía te sirve.')+'</section></div>';
  }
  ...
  function recursoHTML(perfil,a){
    ...
    return '<div class="cpVista">'+volver(id)+cab('Tu recurso de equilibrio','Esta es nuestra versión del cálculo tradicional conocido como “Dios Útil”. Primero mira tu Día Maestro y la estación solar de nacimiento; cuando el método deja más de una ruta, usamos el conteo de tu propia carta para darle contexto.')+
