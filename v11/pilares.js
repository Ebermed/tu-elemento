...
/* Reset único de la fase de pruebas. Base.js ya está cargado, así que al
   terminar dejamos también su marcador de esquema listo para la siguiente
   visita. Esto borra únicamente datos de Tu Elemento. */
try{
  var RESET='tuelemento.v11.reset.11.1';
  if(localStorage.getItem(RESET)!=='1'){
    var borrar=[];
    for(var ri=0;ri<localStorage.length;ri++){
      var rk=localStorage.key(ri);
      if(rk&&(rk.indexOf('tuelemento.')===0||rk.indexOf('tuElemento.')===0||rk.indexOf('te.')===0))borrar.push(rk);
    }
    for(var rj=0;rj<borrar.length;rj++)localStorage.removeItem(borrar[rj]);
    if(typeof TE_SCHEMA_KEY!=='undefined'&&typeof TE_SCHEMA_VALUE!=='undefined')localStorage.setItem(TE_SCHEMA_KEY,TE_SCHEMA_VALUE);
    localStorage.setItem(RESET,'1');
  }
}catch(resetError){console.warn('Tu Elemento V11 · reset',resetError);}

/* V11 runtime: formulario robusto + pilares perezosos.
   Este módulo se carga antes de app.js y captura ambos gestos antes de
   que puedan llegar a los controladores del render principal. */
...
function fallo(msg,el){var e=$('#err');if(e)e.textContent=msg;if(el&&el.focus)el.focus();procesando=false;var b=$('#calcular');if(b){b.disabled=false;b.textContent=$('#paraOtra')&&$('#paraOtra').getAttribute('aria-pressed')==='true'?'Ver su elemento':'Ver mi elemento';}}
function resolverLugar(){var c=$('#buscaLugar');var t=String(c&&c.value||'').trim();if(!t||typeof buscarLugares!=='function')return null;var a=buscarLugares(t,8)||[];if(!a.length){var s=t.split(/[·,]/)[0].trim();a=s?(buscarLugares(s,8)||[]):[];}return a[0]||null;}
function enviar(ev){
...
function activar(i){if(!estado||!estado.panel||!estado.contenidos[i])return;estado.cols.forEach(function(b,n){var on=n===i;b.setAttribute('aria-expanded',on?'true':'false');var mas=b.querySelector('.mas');if(mas)mas.textContent=on?'cerrar':'leer';});estado.panel.innerHTML=estado.contenidos[i];}
function preparar(){
  ...
  estado={lectura:lectura,cols:cols,contenidos:contenidos,panel:panel};activar(0);return true;
}
document.addEventListener('click',function(e){var b=e.target&&e.target.closest?e.target.closest('#lectura .columnas .col'):null;if(!b)return;e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();if(!preparar())return;var i=estado.cols.indexOf(b);activar(i<0?0:i);},true);
})();