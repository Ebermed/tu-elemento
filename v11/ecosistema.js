...
(function(){
  'use strict';
  if(typeof document==='undefined'||globalThis.__TE_ECOSISTEMA_CARGADO)return;
  globalThis.__TE_ECOSISTEMA_CARGADO=true;
  ...
  function getActivo(){var q=param('perfil');if(q)return q;try{return localStorage.getItem(ACTIVO)||'';}catch(e){return'';}}
  function setActivo(id){if(!id)return;try{localStorage.setItem(ACTIVO,id);}catch(e){}refrescarLinks();}
  function conPerfil(url){var id=getActivo();return url+(id?'?perfil='+encodeURIComponent(id):'');}
  function pagina(){var p=location.pathname.toLowerCase();if(/ciclos\.html$/.test(p))return'ciclos';if(/mes\.html$/.test(p))return'mes';if(/calendario\.html$/.test(p))return'calendario';if(/carta\.html$|nueva\.html$|\/v11\/$|\/v11\/index\.html$/.test(p))return'carta';return'inicio';}
  ...
  function refrescarLinks(){var id=getActivo();[['carta','carta.html'],['calendario','calendario.html'],['mes','mes.html'],['ciclos','ciclos.html']].forEach(function(x){var a=document.querySelector('[data-eco="'+x[0]+'"]');if(a)a.href=base+x[1]+(id?'?perfil='+encodeURIComponent(id):'');});}
  function cerrar(){var b=document.querySelector('.teEcoMenuBtn'),l=document.querySelector('.teEcoLinks'),f=document.querySelector('.teEcoBackdrop');if(b)b.setAttribute('aria-expanded','false');if(l)l.classList.remove('abierto');if(f)f.classList.remove('abierto');}
  ...
  if(fondo)fondo.addEventListener('click',cerrar);if(links)links.addEventListener('click',cerrar);addEventListener('resize',function(){if(innerWidth>760)cerrar();});

  var qid=param('perfil');if(qid)setActivo(qid);
  ['#perfilCal','#perfilMes','#perfilCiclos'].forEach(function(sel){var el=document.querySelector(sel);if(el)el.addEventListener('change',function(){setActivo(this.value);});});
  ...
  if(pagina()==='calendario'&&param('vista')==='buscar')setTimeout(function(){var b=document.getElementById('vBuscar');if(b)b.click();},120);
